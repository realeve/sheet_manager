...
const ImageTitle = ({ data: { 缺陷等级, 宏区编号, 开位, 机台, 印刷顺序号, 检测时间 } }) => (
  <div >
    <p style={{ marginBottom: 0 }}>
      宏区{宏区编号} / 第{开位}开 / {缺陷等级}
    </p>
    <p style={{ marginBottom: 0, display: 'flex', justifyContent: 'space-between' }}><span>印刷顺序号：{印刷顺序号}</span><span>{机台}</span></p>
...
const IntagImgs = ({ cart }) => {
  ...
  useEffect(() => {
    setLoading(true)
    db.getViewErrorImage(cart).then(setState).finally(() => {
      setLoading(false);
    })
  }, [cart])

  return <Tabs defaultActiveKey="胶印" >
    {state.map(item => <TabPane tab={item.title} key={item.title}>
      <Skeleton active loading={loading}><ul className={styles.intaglioImg}>
        {item.data.map(subItem => <li
          key={subItem.id}
...
const PrintLine = ({ cart }) => {
  ...
  const params = { renderer: 'canvas', type: 'line', simple: CHART_MODE.SHOW_TITLE, legend: 0, x: 1, y: 2, smooth: true };
  const beforeRender = option => {
    return {
      ...
      grid: {
        left: 55, right: 35, top: 35, bottom: 25
      }
    };
  }
