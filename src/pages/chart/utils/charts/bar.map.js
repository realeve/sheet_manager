...
let handleDataWithLegend = (srcData, option) => {
  let { data } = srcData;
  let getUniqByIdx = idx => R.compose(R.uniq, R.pluck(idx))(data);
  let xAxis = R.sort((a, b) => a - b)(getUniqByIdx(option.x));
  let legendData = getUniqByIdx(option.legend);
  let getSeriesData = name => {
    let dataList = R.filter(R.propEq(option.legend, name))(data);
    let seriesData = R.map(item => {
      let temp = R.find(R.propEq(option.x, item))(dataList);
      return R.isNil(temp) ? "-" : R.nth(option.y)(temp);
    })(xAxis);
    return { data: seriesData, name, type: option.type, smooth: option.smooth };
  };
...
let handleDataNoLegend = (srcData, option) => {
  let { data } = srcData;
  let getDataByIdx = idx => R.pluck(idx)(data);
  ...
  let getSeriesData = () => {
    ...
    return { data: seriesData, type: option.type, smooth: option.smooth };
  };
